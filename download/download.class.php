...
/**
 * @Author: Luhix
 * @Date:   2017-08-31 13:53:16
 * @Last Modified by:   Luhix
 * @Last Modified time: 2017-08-31 14:46:57
 */
...
class download
{
    ...
    function downloadfile($filename)
    {
        ...
        if ($this->filecheck()) {
            $fn = array_pop(explode('/', strtr($this->filename, '\\', '/')));  //下载时的文件名
    ...
    function filecheck()
    {
        ...
        if (file_exists($filename)) {
            $filetype = @strtolower(array_pop(explode('.', $filename))); //array_pop删除数组中的最后一个元素
            echo $filetype;
            if (in_array($filetype, $this->Filter)) {
                ...
                if (!empty($this->mineType)) return true; else {
